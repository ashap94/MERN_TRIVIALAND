...
class RoundThree extends React.Component {
  ...
  removeCategory = (cat) => {
    ...
    if (this.state.allCategoriesNamesArr.length < 1) {
      ...
      this.setState((state) => {
        let showingCategoriesArr = state.showingCategoriesArr.filter(
          (item) => cat !== item
        );
        let allCategoriesNamesArr = state.allCategoriesNamesArr;
        showingCategoriesArr = showingCategoriesArr.concat(
          allCategoriesNamesArr.shift()
        );
        return {
          showingCategoriesArr,
          allCategoriesNamesArr,
  ...
  changeMiniRound(nextRound) {
    ...
    switch (this.state.miniRound) {
      ...
      case 1:
        if (!this.state.currentUsersCategory)
          this.chooseCategory(this.state.showingCategoriesArr[0]);
  ...
  render() {
    ...
    let playersTurnDisplay = this.state.currentUserTurn ? (
      <div className="rnd-3-category-container">
        ...
        <h1
          className={
            this.state.miniRound === 1
              ? "rnd-3-cat-instructs instructs-bold"
              : "rnd-3-cat-instructs"
          }
        >
          1: You have 10 Seconds to choose your category
        </h1>
        <h1
          className={
            this.state.miniRound === 2
              ? "rnd-3-cat-instructs instructs-bold"
              : "rnd-3-cat-instructs"
          }
        >
          2: You have 10 Seconds to choose your wager
        </h1>
        <h1
          className={
            this.state.miniRound === 3
              ? "rnd-3-cat-instructs instructs-bold"
              : "rnd-3-cat-instructs"
          }
        >
          3: You have 30 Seconds to Answer
        </h1>
        {this.state.currentUsersCategory ? (
          <div className="rnd-3-cat-selector">
            {this.state.currentUsersCategory}
          </div>
        ) : (
          this.state.showingCategoriesArr.map((category) => (
            <div
              className="rnd-3-cat-selector"
